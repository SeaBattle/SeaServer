...
form_full_rules_test() ->
  ...
  RulesEnc = ss_rules_logic:encode_rules(Rules),
  ?assertEqual(Rules, ss_rules_logic:decode_rules(RulesEnc)).
...
form_partial_rules_test() ->
  ...
  ?assertEqual(RulesFull, ss_rules_logic:decode_rules(RulesEnc)).
...
form_default_rules_test() ->
  ...
  DefaultEnc = ss_rules_logic:encode_rules(#{}),
  ?assertEqual(Default, ss_rules_logic:decode_rules(DefaultEnc)).
...
incorrect_ship_num_test() ->
  ...
  Res = try ss_rules_logic:encode_rules(Rules)
        catch throw:Err -> Err
        end,
